...
class DatabaseService extends GetxService implements Bootable, DatabaseServiceImpl {
  ...
  @override
  Future<Either<Failure, GroupChatModel>> getGroupChatByGroupChatId({required String groupChatId}) async {
    try {
      final querySnapshot =
          await _db.collection(DatabaseKeys.groupChat).where('groupChatId', isEqualTo: groupChatId).limit(1).get();
      if (querySnapshot.docs.isNotEmpty) {
        final groupChatModel = GroupChatModel.fromJson(querySnapshot.docs.first.data());
  ...
  @override
  Stream<QuerySnapshot<Map<String, dynamic>>> listenMessageChatByGroupChat({required String groupChatId}) => _db
  ...
  @override
  Future<Either<Failure, CommentModel>> getCommentByOrderId({required String orderId}) async {
    try {
      final querySnapshot =
          await _db.collection(DatabaseKeys.commentPath).where('orderId', isEqualTo: orderId).limit(1).get();
  ...
  Stream<QuerySnapshot<Map<String, dynamic>>> listenRating() =>
      _db.collection(DatabaseKeys.commentPath).orderBy('createAt', descending: true).snapshots();
  ...
  @override
  Stream<QuerySnapshot<Map<String, dynamic>>> listenCurrentOrder(String userId) => _db
  ...
  Future<Either<Failure, List<OrderModel>>> getListOrderModelByStatus(
      {HistoryStatus? status, required String timeStampStart, required String timeStampEnd}) async {
    try {
      List<OrderModel> _result = [];
      var querySnapshot;
      if (status == null) {
  ...
  Stream<QuerySnapshot<Map<String, dynamic>>> listenNotification({required String userId}) =>
      _db.collection(DatabaseKeys.notificationPath).where('receiverId', isEqualTo: userId).snapshots();
