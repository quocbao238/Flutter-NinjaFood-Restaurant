...
class OrderItems extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final listStatus = adminHomeController.getListStatus(orderModel: orderModel);
                              ...
                              children: [
                                Expanded(
                                  child: Column(
                                    ...
                                    children: [
                                      AppText.bodyMedium(
                                          text: 'Code_Order'.tr + ' ${orderModel.createdAt}',
                                          maxLines: 1,
                                ...
                                AppPadding(
                                  ...
                                  child: AppText.bodyLarge(
                                      text: totalPrice, color: ThemeColors.textPriceColor, fontWeight: FontWeight.bold),
                                ),
                              ],
                            ),
                          ),
                            ...
                            child: Row(
                              ...
                              children: [
                                ...
                                PopupMenuButton<HistoryStatus>(
                                  enabled: orderModel.status != HistoryStatus.done,
                                  shape: RoundedRectangleBorder(
                                      borderRadius: BorderRadius.circular(18),
                                      side: BorderSide(color: orderModel.status.color, width: 1)),
                                  padding: EdgeInsets.zero,
                                  splashRadius: 18,
                                  color: Theme.of(context).colorScheme.onPrimary,
                                  itemBuilder: (BuildContext context) {
                                    return listStatus.map((HistoryStatus historyStatus) {
                                      return CustomPopupMenuItem<HistoryStatus>(
                                        value: historyStatus,
                                        showDivider: historyStatus != listStatus.last,
                                        child: AppText.bodySmall(text: historyStatus.adminStatus.tr),
                                        onTap: () {
                                          adminHomeController.updateStatusOrder(
                                              orderModel: orderModel, status: historyStatus);
                                  ...
                                  child: Container(
                                    constraints:
                                        BoxConstraints(minWidth: MediaQuery.of(context).size.shortestSide * 0.2),
                                    child: DecoratedBox(
                                      decoration: BoxDecoration(
                                          ...
                                          border: Border.all(color: orderModel.status.color, width: 1),
                                          borderRadius: BorderRadius.circular(18),
                                          color: orderModel.status.color.withOpacity(0.05)),
                                      child: AppPadding.small(
...
class CustomPopupMenuItem<T> extends PopupMenuEntry<T> {
  ...
  CustomPopupMenuItem({required this.value, required this.child, this.onTap, this.showDivider = true});
