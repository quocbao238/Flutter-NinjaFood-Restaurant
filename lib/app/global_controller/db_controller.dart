...
class DatabaseController extends GetxService {
  ...
  Future<Either<Failure, List<ProductModel>>> getListProductById(List<int> listProductsIds) async {
    try {
      List<ProductModel> _result = [];
      final querySnapshot = await _db.collection(DatabaseKeys.product).where('id', whereIn: listProductsIds).get();
      _result = querySnapshot.docs.map((e) => ProductModel.fromJson(e.data())).toList();
      return right(_result);
    } catch (e, stackTrace) {
      return left(Failure(e.toString(), stackTrace));
